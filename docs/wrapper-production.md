...
9. Each Function enforces its documented method before source identity, rate/concurrency state, body consumption, or backend access. Wrong methods return sanitized `405 method_not_allowed` with the endpoint's `Allow` header; pre-backend Action rejections remain explicitly non-invalidating.
...
- Cross-origin redirects, subframes, popups, downloads, uploads, service workers, WebSockets, EventSource, WebRTC, Beacon, form submission, and non-reading methods are blocked. A server-trusted CDP main-frame identity distinguishes main-document redirects from blocked subframes; every actual main-document redirect hop during explicit navigation is independently checked for consequential path/query evidence before it can reach the target origin. After every action kind, the final page URL is checked twice for the exact validated origin and independently decoded consequential path, query, or fragment evidence; malformed escapes fail closed. This prevents `pushState`, hash-router checkout routes, and settle-time changes from bypassing request-level guards while neutral same-document fragments remain allowed.
- Preparation actions set the context offline before page mutation. They never submit and allow zero page network requests.
- DOM safety classification and native control operations run in a CDP isolated world. Server-only backend-node identities bind actions and verification to the classified element; page-visible marker attributes and mutable main-realm prototypes are not trust anchors. Controls and links must have a CDP paint-hit-tested visible pixel inside the fixed 1365×900 screenshot viewport after clip-path and every ancestor overflow clip, both during classification and immediately before state reads, writes, or verification; opaque `pointer-events:none` overlays therefore cannot expose a visually covered control. Controls are rechecked for effective `:disabled` and read-only state before every read and write. Select mappings inspect at most 200 native options and retain at most 30 effectively enabled options whose bounded label attribute, actual text, and value all pass safety classification; any incomplete traversal or additional enabled option excludes the Select. Multi-select controls are excluded and the native `multiple` state is revalidated immediately before every state operation. A shared isolated-world snapshot revalidates these sources immediately before reads, writes, link navigation, and verification. Non-`none` CSS filters or masks on the element or any ancestor are conservatively excluded because their visible pixels cannot be established safely from layout geometry alone.
- Sensitive-field classification independently checks bounded `aria-labelledby` and `aria-describedby` targets (HTML, SVG, and other elements), every bounded descendant image alt from every associated label, label text, aria-label, title, placeholder, name, ID, autocomplete, and relevant link evidence. At most 16 references, labels, or label images and 4 KiB of safety text are inspected per source, with an additional hard 24 KiB cumulative retained-evidence/snapshot budget per control. Any count, traversal, individual, or aggregate overflow marks the control unsafe instead of truncating away a sensitive signal. The same exact snapshot is revalidated at action time.
- Text, search, and textarea tools mirror native length contracts conservatively: native `required` raises the effective minimum to one in schema, sample, server validation, and isolated-world validation; public Unicode-code-point limits are chosen so every accepted value also fits the browser's UTF-16 limit, and samples must differ from the analyzed value. Non-empty native `pattern` controls are excluded instead of publishing an incomplete schema. Numeric, Select, and radio samples likewise choose a safe state different from the analyzed state; controls without an executable alternative are omitted from samples and tools. Radio groups are exposed only when every native same-name/form-owner member is visible, non-sensitive, server-bound, and revalidated as a complete group before every state operation.
...
- Date, month, time, and week inputs are exposed only when Chromium can enumerate their complete native min/max/step value set within 200 entries and that set contains a value different from the analyzed native value. The exact finite enum and executable alternative are used for the public schema, sample, pre-action validation, and isolated-world retained-value check. Current-only controls are omitted. Checkbox samples invert the browser-native analyzed state so the advertised invocation produces a visible change; an initially or subsequently indeterminate checkbox is excluded or rejected before a verified success.
...
- Every accepted action is raced against one absolute, non-extendable session deadline through delayed start, navigation/network settling, mutation, verification, reanalysis, and final response construction. Queued actions recheck the same deadline immediately after acquiring their turn. Expiry destroys the session, releases the queue, and never returns Activity or expired credentials; the inner Playwright deadline is clamped to the earlier worker-owned Sandbox/config deadline.
- Request bodies are capped at 32 KiB, serialized responses at 2 MiB, screenshots at 900 KiB, DOM traversal at 5,000 inspected elements, DOM evidence at 80 controls, Select traversal at 200 options, and AX evidence at 40 nodes. Partial AX queries are issued only for the at most 80 retained DOM controls. Analysis is capped at one absolute 35-second deadline and Production actions at one absolute 15-second deadline beginning before reconnect and covering worker command/output, response parsing, decoration, final return checks, and fail-closed cleanup. Untrusted target downloads have a separate 4 MiB per-resource and 20 MiB cumulative session budget. Response headers are rejected early and CDP continuously measures both transferred and decoded bytes; a breach stops loading, takes the context offline, and invalidates the session.
